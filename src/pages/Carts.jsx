...
export default function Carts() {
		...
		try {
			...
			if (carts.find((item, index) => item[index] === cart[index])) {
				body = carts.map((item) => ({
					...item,
					qty:
						item.product.name === cart.name && item.qty !== 0
							? item.qty - 1
							: item.qty,
				}));
			} else {
	...
	const plusQty = async (cart) => {
		try {
			...
			if (carts.find((item, index) => item[index] === cart[index])) {
				body = carts.map((item) => ({
					...item,
					qty: item.product.name === cart.name ? item.qty + 1 : item.qty,
				}));
			} else {
				body = [...carts];
			}
			...
			setCarts(body);
			await axios.put(`${api}/api/carts`, { items: body }, authHeaders);
		} catch (err) {
			console.log(err);
		}
	};
	...
	const checkOut = async () => {
		try {
			...
			}; // synchronizing the format data equal to the model at the backend

			if (selectedAddress.name !== 'select one: ') {
				await axios.post(`${api}/api/orders`, body, authHeaders).then((res) => {
			...
			<div className='container'>
				<div className='min-h-screen px-4 py-4 bg-sky-50'>
					...
					<div className='flex items-center justify-between'>
						<div className='flex justify-center w-full pt-4 pb-2 text-2xl uppercase'>
							Your Carts
						</div>
					...
					<div className='px-4 py-4 my-2 bg-white rounded-md shadow-sm'>
						<div className='flex items-center justify-between mb-2 gap-x-4'>
							...
							<div className='flex-auto px-2 py-1 border rounded-md shadow-sm'>
								free, promo
							</div>
						</div>
						<div className='flex items-start justify-between gap-x-4'>
							...
							<div className='flex-auto'>
								<Listbox value={selectedAddress} onChange={setSelectedAddress}>
									<Listbox.Button
										className={`${
											selectedAddress.name === 'select one: '
												? 'bg-red-200'
												: 'bg-blue-200'
										} rounded-md border px-2 hover:font-semibold py-1 w-full shadow-sm justify-start hover:ring hover:ring-sky-200`}
									>
										{selectedAddress.name}
									</Listbox.Button>
									<Listbox.Options
										className={
											'w-full mt-1 overflow-hidden bg-white border border-gray-200 rounded-md'
										}
									>
										{addressData.map((address) => (
											<Listbox.Option
												key={address._id}
												value={address}
												className='px-2 py-0.5 cursor-pointer hover:bg-sky-200'
											>
						...
						{addressData.length !== 0 ? (
							...
							<div className='text-right text-red-500'>
								You dont have any address yet, please add a new one.
							</div>
						)}
						<div className='flex items-center justify-end mt-2 gap-x-2'>
							...
							<Link
								to='/user/address'
								className='flex justify-center px-2 py-1 bg-green-300 rounded-md shadow-sm w-44 hover:font-semibold hover:ring hover:ring-green-300 hover:bg-green-400'
							>
								Add New Address
							</Link>
						</div>
					...
					<div className='flex flex-col gap-4 px-4 py-4 bg-white rounded-md shadow-sm'>
						{carts.length === 0 ? (
							<div className='px-4 py-2 bg-red-200 rounded-md'>
								there is no product in here
							</div>
						) : (
							carts.map((cart) => (
								<div
									key={cart._id}
									className='flex flex-row items-center overflow-hidden border rounded-lg bg-sky-100'
								>
									<div className='w-56 bg-white h-44'>
										<img
											className='object-cover'
											src={`${api}/api/images/${cart.img_url}`}
											alt='product'
										/>
									</div>
									<div className='w-full px-4'>
										<div className='flex justify-end'>
											...
											<button
												className='px-2 py-0.5 text-red-400 hover:text-red-700'
										...
										<div className='flex items-center justify-end gap-x-2'>
											<button
												className='px-2 py-2 text-gray-700 bg-white rounded-md shadow-sm hover:bg-red-400'
												onClick={() => minQty(cart)}
											>
												<svg
													xmlns='http://www.w3.org/2000/svg'
											...
											<div>{cart.qty}</div>
											<button
												className='px-2 py-2 text-gray-700 bg-white rounded-md shadow-sm hover:bg-green-400'
												onClick={() => plusQty(cart)}
											>
												<svg
													xmlns='http://www.w3.org/2000/svg'
													className='w-4 h-4'
						...
						<div className='flex justify-end -mt-3'>
							{selectedAddress.name === 'select one: ' ? (
								<p className='mb-2 text-red-400'>
									please select your shipping address first
								</p>
							) : (
								''
							)}
							{validation.error === 1 ? (
								<p className='mb-2 text-red-400'>{validation.message}</p>
							) : (
								''
							)}
						</div>
