...
export default function Index() {
    ...
    const getCountCart =  async () => {
        try {
            await axios
                ...
                .then( res => {
                    setCountCart(res.data.map( item => item.qty).reduce((preVal, currentVal) => preVal + currentVal, 0))//
                    setCarts(res.data);
                })
        } catch (err) {
    ...
    const addCarts = async (product) => {
        try {
            const { category, tags, description, createdAt, updatedAt, __v, _id, ...remainProduct  } = product; //this step is to put apart : name, price, img_url
            let newDataProduct = remainProduct;

            if(carts.find( item => item.product._id === _id)) {
                newDataProduct = carts.map( item => ({ ...item, qty: item.product._id === _id ? item.qty + 1 : item.qty})); //only adding 1 qty if the data is already exist  
            } else {
                newDataProduct = [ ...carts, { qty: 1, product: product, user: userDetail, ...remainProduct}]; //adding new data if the data is not exist....  note : product is from parameter product, userDetail is from useState above
            }

            let dataCountCarts = newDataProduct.map( item => item.qty).reduce((preVal, currentVal) => preVal + currentVal, 0) //summing of all the qtys amount 
            setCountCart(dataCountCarts); //changing the count carts 
            setCarts(newDataProduct); //changing the state carts 

            await axios.put(`${api}/api/carts`, {items: newDataProduct}, authHeaders) // send the data to the api whenever the carts value is changing
    ...
    const logout = async (e) => {
        ...
        try {
            await axios
                .post(`${api}/auth/logout`, null, authHeaders)
                .then(res => {
                    if(res.data.error !== 1) {
    ...
    const getPage = async () => {
        try {
            ...
            let selectedTags = []
            if(sendTag.length !== 0){
                selectedTags = sendTag.map( (item, index) => (`tags[${index}]=${item.name}`)).join('&')
            } else {
                selectedTags = ''
            }
            ...
            let sendLinkApi = `${linkApi}${selectedCategory !== '' ? `${selectedCategory}&&` : ''}${selectedTags !== '' ? `${selectedTags}&&` : ''}${selectedQuery !== '' ? selectedQuery : ''}${skip !== '' ? skip : ''}`

            await axios(sendLinkApi)
                .then( res => setProducts(res.data))
                        ...
                        { !!userDetail.full_name ? (
                            <div className='flex items-center text-white gap-x-2'>
                                ...
                                <Menu as={'div'} className='relative'>
                                    <Menu.Button className={`${userDetail.role === 'admin' ? 'font-bold' : ''} relative capitalize justify-end flex items-center px-2 py-1 rounded-lg hover:bg-blue-700/40 gap-x-3`}>
                                        {`${userDetail.full_name}`} 
                                    ...
                                    <Menu.Items as={'div'} className='absolute items-center w-full mt-2 overflow-hidden text-gray-700 bg-white rounded-md'  >
                                        ...
                                        <Menu.Item className='block w-full px-4 py-2 hover:text-black hover:bg-red-100' onClick={(e) => logout(e)}>
                                            <a href='/'>Logout</a>
                                        </Menu.Item>
                                    </Menu.Items>
                                </Menu>
        ...
        <div className='bg-sky-200 '>
            <div className='container'>
                <div className='bg-sky-50'>
                    <div className='flex items-center justify-center pt-2 mx-auto'>Tags:</div>
                            ...
                            products.length !== 0 ?  
                                <div className='grid grid-cols-5 gap-4 mx-6 my-4'>
                                    {products.data.map( product => (
                                        <div key={product._id}>
                                            <Card dataProduct={product} dataUser={userDetail} onClick={ () => addCarts(product)} />
                                        </div>
                                    ))}
                                </div>
                                : <div className='flex justify-center h-full mx-4 bg-white'>
                                    <p>loading</p>
                                </div>
                        }
                    <hr className='mx-5 border-gray-300' />
